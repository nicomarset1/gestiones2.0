    ...
    <main className="min-h-screen overflow-hidden bg-[#080808] text-zinc-100">
      ...
      <div className="relative z-10 mx-auto grid min-h-screen max-w-6xl items-center gap-10 px-5 py-16 lg:grid-cols-[1fr_0.9fr]">
        ...
        <Panel className="p-6">
          ...
          <h2 className="mt-3 text-3xl font-semibold text-white">{mode === "login" ? "Entrá a tu espacio" : "Creá tu espacio de trabajo"}</h2>
        ...
        <Panel className="p-6">
          ...
          {data.clients.length === 0 ? (
            ...
            <form onSubmit={submit} className="mt-6 space-y-4">
              ...
              <div className="flex gap-3"><PrimaryButton type="submit" className="flex-1">{editing ? "Guardar cambios" : "Crear presupuesto"}</PrimaryButton>{editing && <SecondaryButton onClick={reset}>Cancelar</SecondaryButton>}</div>
            </form>
          )}
        </Panel>
        ...
        <Panel className="p-6">
          ...
          {data.clients.length === 0 ? (
            ...
            <form onSubmit={submit} className="mt-6 space-y-4">
              ...
              <Input label="Observaciones" value={form.observations} onChange={(v) => setForm({ ...form, observations: v })} placeholder="Opcional" />
              <div className="grid grid-cols-2 gap-3"><Select label="Estado" value={form.status} onChange={(v) => setForm({ ...form, status: v })} options={ORDER_STATUS} /><Select label="Pago" value={form.payment} onChange={(v) => setForm({ ...form, payment: v, paidAmount: v === "Pagado" ? form.total : v === "Pendiente" ? "" : form.paidAmount })} options={PAYMENT_STATUS} /></div>{form.payment === "Pago parcial" && <Input label="Monto pagado" value={form.paidAmount} onChange={(v) => setForm({ ...form, paidAmount: formatARSInput(v) })} placeholder="Ej: 40.000" />}
              ...
              <div className="flex gap-3"><PrimaryButton type="submit" className="flex-1">{editing ? "Guardar cambios" : "Crear orden"}</PrimaryButton>{editing && <SecondaryButton onClick={reset}>Cancelar</SecondaryButton>}</div>
            </form>
          )}
        </Panel>
